...
void drawThumbnails(int windowWidth, int windowHeight) {
	float thumbnailSize = windowWidth / 10.0f;
	for (int i = 0; i < 10; i++) {
...
void display( void )
{
	...
	int windowHeight = glutGet(GLUT_WINDOW_HEIGHT) - 6;
	float thumbnailSize = windowWidth / 10.0f;
	float tiledImgWidth;
	...
	case STATE_POLYLINE_VIEWER:
		...
		drawPolylineDrawing(currentDrawing, GL_LINE_STRIP, program, 0.0f, 0.0f, (float) windowWidth, (float) (windowHeight - thumbnailSize));
		break;

	case STATE_POLYLINE_DRAW:
		drawThumbnails(windowWidth, windowHeight);
		drawPolylineDrawing(drawing, GL_LINE_STRIP, program, 0.0f, 0.0f, (float)windowWidth, (float)(windowHeight - thumbnailSize));
		break;

	case STATE_POLYLINE_TILES:
		drawThumbnails(windowWidth, windowHeight);

		tiledImgWidth = windowWidth / 5.0f;
		tiledImgHeight = (windowHeight - thumbnailSize) / 5.0f;
		srand(seed);
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
	...
	case STATE_FRACTAL_GINGERBREAD:
		drawGingerbreadMan(program, 0, 0, (float) windowWidth, (float) (windowHeight - thumbnailSize));
		break;

	case STATE_FRACTAL_FERN:
		drawFern(program, 0, 0, (float)windowWidth, (float)(windowHeight - thumbnailSize));
		break;
...
void goToPicture(int x, int y, float windowWidth) {
...
void mouse(int button, int state, int x, int y) {
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		...
		case STATE_POLYLINE_DRAW:
			...
			addPointToCurrentLine(drawing, x, (windowHeight - y), 0, 0, windowWidth, windowHeight - (windowWidth / 10.0f));
			display();
			break;
